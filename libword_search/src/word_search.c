...
bool word_search__search(
    ...
    if( out__solutions == NULL ){
        error__set(
            ...
            "word_search",
            WordSearch__ErrorType__NullReference,
            "word_search__search: Parameter out__solutions cannot be NULL."
        );
    ...
    if( out__solutions->capacity < words.length ){
        error__set(
            ...
            "word_search",
            WordSearch__ErrorType__NullReference,
            "word_search__search: Parameter out__solutions does not have sufficient capacity to store solutions for all words. "
            "Word count: %llu. Capacity of parameter out__solutions: %llu.\n",
            words.length,
            out__solutions->capacity
        );
    ...
    for( unsigned long word_index = 0; word_index < words.length; word_index++ ){
        out__solutions->data[ word_index ] = word_search__find_word( words.data[ word_index ], grid, grid_hash_map );
        out__solutions->length += 1;
    }
